...
"""
Creators/authors:
Sanna Bashir 2022445
Geanina Verestiuc 2026936
Aidan Ward 2024085
"""
...
RED = (255, 0, 0)
GREEN = (0, 255, 0)
TURQUOISE = (44, 149, 188)
PASTEL_ORANGE = (233, 165, 111)
LIGHT_BLUE = (130, 231, 249)
ORANGE =(255, 126, 0)
DARK_GREEN  = (9, 30, 37)
...
#initializing fonts for start and pause screen
f1 = pg.font.SysFont("Arial", 35)
f2 = pg.font.SysFont("Arial", 60)
f3 = pg.font.SysFont("Arial", 80)
...
class Player(pg.sprite.Sprite):
    ...
    def update(self):
        ...
        """need to call on functions to be able to happen every iteration"""
        self.adjust_collision_bottom() 
        self.player_hit()
        self.enemy_hit()
        self.enemy_hitpu()
...
#zombie animations
zombie_left =[pg.image.load( path + f'ZomL{frame}.png').convert_alpha() for frame in range(6)]
zombie_right = [pg.image.load( path + f'ZomR{frame}.png').convert_alpha() for frame in range(6)] #enemy animation
...
#animation
ork_left =  [pg.image.load( path + f'WL{frame}.png').convert_alpha() for frame in range(7)]
ork_right =  [pg.image.load( path + f'WR{frame}.png').convert_alpha() for frame in range(7)]
...
class Ork(pg.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.rect = self.image.get_rect()
        self.rect.x = rd.randrange(1100, 1900)
        self.rect.y = HEIGHT - ty - self.rect.height + 5
...
#animations
bigboss_left = [pg.image.load( path + f'BbL{frame}.png').convert_alpha() for frame in range(12)]
bigboss_right  =[pg.image.load( path + f'BbR{frame}.png').convert_alpha() for frame in range(12)]
...
bg2 =  pg.image.load( path + 'bg2.jpg').convert_alpha()
bg2_rect = bg2.get_rect()


"""----------------------------------------
designing a start, pause and end screen
-----------------------------------------"""

def text_objects(text, color, size):
    """here is where we develop fonts to render"""
    if size == "small":
        textSurf = f1.render(text, True, color)
    elif size == "medium":
        textSurf = f2.render(text, True, color)
    elif size == "large":
        textSurf = f3.render(text, True, color)
    
    return textSurf, textSurf.get_rect()
    
    
def textbutton(text, color, x, y, width, height, size="small"):
    """this is where we add the text object to a button so it isn't seperate and will move with the button"""
    textSurf, textRect = text_objects(text, color, size) 
    textRect.center = (x +(width/2), y + (height/2)) #here we position the text on the button
    window.blit(textSurf, textRect)

    
def message(text, color, x, y, size = "small"):
    """develops a message to paste on screen, to the console"""
    textSurf, textRect = text_objects(text, color, size)
    textRect.center = (x, y)
    window.blit(textSurf, textRect)
    

def button(text, x, y, w, h, inactive, active, action=None):
    """
    here we create an interactive button that the mouse can click on.
    different buttons lead to different actions, which in turn cause different redirectories
    i equals inactive color, a equals active color
    """
    mouse = pg.mouse.get_pos()
    click = pg.mouse.get_pressed()
    
    if x + w > mouse[0] > x and y + h > mouse[1] > y: #here is where the mouse can click, within the binds of the button shape.
        pg.draw.rect(window, active, (x, y, w, h))
        if click[0] == 1: #these are our actions to play if a button is clicked. It references a specific actions when later referenced.
            if action == "play":
                start.play()
                gameloop()
            elif action == "quit":
                pg.quit()
                sys.exit()
            elif action == "end_screen": #here we are closing the gameloop(), and sending the user to end screen
                end_screen()
    else: 
        pg.draw.rect(window, inactive, (x, y, w, h))
        
    textbutton(text, BLACK, x, y, w, h)

    
def start_screen():
    """here we create our initial start screen, where the player is first directed"""
    window.fill(BLACK)
    window.blit(bg2, bg2_rect)
    message("Graveyard Archeress", ORANGE, 500, 200, "large")
    message("Press Return to play or Q to quit.", LIGHT_BLUE, 500, 300)
    pg.display.update()
    
    #this redirects the player to the menu screen
    intro = True
    while intro:
...
def menu():
    """creates the menu that pops up after the start screen""" 
    intro = True
    while intro: 
        for event in pg.event.get():  #quit function in case user wants to exist
            if event.type == pg.QUIT:
                pg.quit()
                sys.exit()
            if event.type == pg.KEYDOWN: 
        ...
        window.blit(bg2, (bg2_rect)) #we create a background
        
        message("Control using the arrow or WASD keys.", ORANGE, 500, 200, "small")
        message("Press 'P' to pause the game", ORANGE, 500, 230, "small")
        message("Press the space bar to shoot.", ORANGE, 500, 260, "small")
        message("Press 'E' or '/' for powerup attack.", ORANGE, 500, 290, "small")
          
        button("Start Game", 150, 330, 300, 150, TURQUOISE, PASTEL_ORANGE, "play") #these here are the buttons for play and quit, as appears on the screen
        button("Quit", 550, 330, 300, 150, TURQUOISE, PASTEL_ORANGE, "quit") 
    
        pg.display.update()
        clock.tick(15)

def pause():
    """creates a pause screen and function"""
    paused = True
    while paused: 
        for event in pg.event.get():
            if event.type == pg.QUIT:
            ...
            if event.type == pg.KEYDOWN: 
                if event.key == ord('p'):
                    paused = False
                elif event.type == pg.QUIT or event.key == ord('q') or event.key == pg.K_ESCAPE:
                    pg.quit()
                    sys.exit()
        ...
        message("Pause", PASTEL_ORANGE, 500, 200, "large")
        
        button("End Game", 150, 330, 300, 75, TURQUOISE, PASTEL_ORANGE, "end_screen")
        button("Continue", 550, 330, 300, 75, TURQUOISE, PASTEL_ORANGE, "play")
...
def end_screen():
    ...
    while intro: 
        ...
        message("Thanks for Playing", PASTEL_ORANGE, 500, 100, "large")
        message("Creators: Sanna Bashir, Geanina Verestiuc and Aidan Ward", ORANGE, 500, 155, "small")
        message("Your high score: " + str(player.score), LIGHT_BLUE, 500, 250, "medium")
        
        button("Close Window", 320, 400, 350, 75, TURQUOISE, PASTEL_ORANGE, "quit")
...
def gameloop():
    ...
    while running:
        ...
        """events taken from the user input, decides movement, pausing, shooting and quit screen"""
        ...
        #if the enemy's attack hits the player or disappeares off the screen, then the enemy can shoot again
        for bb in bigboss:
            if not attacks:
                bb.canshoot = True
        ...
        """update section: side scroller control"""
        if not zombies:
        ...
        text1 = font1.render("Streak:  " + str(player.streak) , 1, (PASTEL_ORANGE)) #save the score
        window.blit(text1, (20,20)) #blit the score on the screen

        font = pg.font.SysFont('Arial', 36) #decide on a font
        text = font.render("Score:  " + str(player.score) , 1, (PASTEL_ORANGE)) #save the score
        window.blit(text, (870,20)) #blit the score on the screen

        #visual stufff behind the sprites
        window.blit(tomb, (530, HEIGHT - ty - 35))
        window.blit(tree, (800, HEIGHT - ty - 120 ))
        window.blit(tomb1, (80, HEIGHT -  ty - 37))
        
        """draw section: drawing all the sprites and healthbars on the screen"""
        if player.health > 0:
            player.healthbar(window)
            if not bigboss:
                font3 = pg.font.SysFont('Arial', 70) 
                text3 = font3.render('YOU  WIN' , 1, (GREEN)) #when the last enemy dies, show the player that they won
                window.blit(text3, (370,250))
                winSound.play() #winning sound effect
                button("Click for End Screen", 360, 300, 250, 35, GREEN, PASTEL_ORANGE, action= "end_screen") #button to redirect the player to the end screen
        else:
            ...
            button("Click for End Screen", 430, 300, 140, 35 , RED, PASTEL_ORANGE, action= "end_screen") #button to redirect player to end screen
...
def run_game(): #function that runs the game
    start_screen()
    menu()
    gameloop()
